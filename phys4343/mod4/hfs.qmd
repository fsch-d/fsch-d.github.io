...
The dominant contribution to the hyperfine structure in many atoms is the
interaction between the **magnetic moment of the nucleus** and the magnetic
field produced by the electrons at the position of the nucleus.
...
\vec{\mu}_I
=
g_I \mu_N \frac{\vec I}{\hbar},
$$

where $g_I$ is the nuclear $g$-factor and
...
The electronic angular momentum produces an effective magnetic field
$\vec B_J$ at the nucleus. The magnetic-dipole interaction is therefore

$$
\hat H_{\mathrm{HF}}
=
-\vec{\mu}_I\cdot\vec B_J.
$$

For an atomic state with a well-defined total electronic angular momentum
$J$, there is no preferred direction other than $\vec J$. The effective
electronic magnetic field must therefore be parallel (or antiparallel) to
$\vec J$.

Recall that the magnitude of an angular momentum with quantum number $J$ is
...
$$
\frac{\vec J}{\sqrt{J(J+1)}\,\hbar}
$$

is a unit vector in the direction of $\vec J$. We can therefore write the
effective electronic magnetic field as
...
where $B_J$ denotes the magnitude of the effective magnetic field at the
nucleus.


Substituting this expression and the nuclear magnetic moment into the
interaction Hamiltonian gives
...
\cdot
\left(
B_J\frac{\vec J}{\sqrt{J(J+1)}\,\hbar}
\right)
\\[4pt]
&=
-
\frac{g_I\mu_N B_J}{\sqrt{J(J+1)}}
\frac{\vec I\cdot\vec J}{\hbar^2}.
\end{aligned}
$$

This motivates the definition of the **magnetic-dipole hyperfine constant**
...
so that the magnetic-dipole hyperfine Hamiltonian takes the compact form
...
=
\frac{A}{\hbar^2}\,
\hat{\vec I}\cdot\hat{\vec J}
}.
$$

The constant $A$ contains the details of both the nucleus and the electronic
wavefunction. In particular, it depends on the nuclear magnetic moment and
on the magnetic field produced by the electrons at the nucleus.

::: callout-note
## What determines $B_J$?

The effective field $B_J$ depends on the electronic state. For an
$s$-electron, an important contribution is the **Fermi contact interaction**,
which is proportional to the electron probability density at the nucleus,

$$
B_J \propto |\psi(0)|^2.
$$

Thus, the hyperfine constant $A$ is sensitive to both nuclear properties and
the electronic wavefunction close to the nucleus. Accurate values of $A$
are obtained from atomic-structure calculations or from experiment.
:::
...
$$
\hat{H}_{\mathrm{HF}} \;=\; \frac{A}{\hbar^2}\,\hat{\vec{I}}\cdot\hat{\vec{J}}
\;=\; \frac{A}{2\hbar^2}\Big(\hat{\vec{F}}^{\,2} - \hat{\vec{I}}^{\,2} - \hat{\vec{J}}^{\,2}\Big).
$$
