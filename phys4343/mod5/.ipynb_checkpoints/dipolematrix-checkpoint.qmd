---
title: "From Relations to Values: Dipole Matrix Elements"
...
1. Start from the atom–field Hamiltonian and introduce the electric dipole approximation for optical transitions.  
2. Use time-dependent perturbation theory to compute transition probabilities between stationary states.  
3. Identify the Einstein coefficients in the perturbative transition rates and thereby obtain explicit expressions for $A_{21}$ and the $B$-coefficients in terms of dipole matrix elements.  
4. Compare the quantum result to the radiation rate of a classical oscillating dipole and understand the origin of the common prefactors.  
5. Draw practical conclusions: estimate lifetimes, understand selection rules, and see why simple two-level pumping cannot produce inversion without additional levels or mechanisms.
...
1. **Start from classical electrodynamics:** express the atom–field interaction in terms of the electromagnetic potentials.  
2. **Apply the dipole approximation:** for optical transitions, the wavelength is typically much larger than the atomic dimensions, so the interaction simplifies to $-\hat{\vec{d}}\cdot \vec{E}$, where $\hat{\vec{d}}$ is the electric dipole operator.  
3. **Use time-dependent perturbation theory:** insert the interaction Hamiltonian into the time-dependent Schrödinger equation and evaluate transition probabilities between quantum states.  
4. **Extract Einstein coefficients:** compare the resulting transition rates with Einstein’s definitions to identify explicit formulas for $A_{21}$, $B_{12}$, and $B_{21}$.  
5. **Check against the classical oscillator model:** verify the quantum result by comparing it with the radiation rate of a classical oscillating dipole, providing intuition and consistency.
...
From Fermi’s Golden Rule, the transition rate between an initial state $|i\rangle$ and a final state $|n\rangle$ depends on the square of the dipole matrix element:

$$
\Gamma_{i \to n} \;\propto\;
|\langle \phi_n | \hat{\vec{\varepsilon}}\cdot \hat{\vec{r}} | \phi_i \rangle|^2.
$$

Summing over all photon directions and polarizations, one obtains the explicit expressions for the Einstein coefficients.

---

## 4.2 Absorption and stimulated emission coefficients

The absorption coefficient $B_{12}$ and the stimulated emission coefficient $B_{21}$ are given by (for equal degeneracy)
...
- The result shows that both coefficients are directly proportional to the squared dipole matrix element.
