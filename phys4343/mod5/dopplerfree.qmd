...
```
![Two-level and three-level $\Lambda$ systems.](./tl_lambda.svg){fig-alt="Two-level and three-level $\Lambda$ systems." fig-align="center" width=40%}
...
with open(os.devnull, 'w') as FNULL:
    ...
    subprocess.run(["rm", "diagram.tex", "diagram.pdf", "diagram.log", "diagram.aux"], stdout=FNULL, stderr=FNULL)
```
![Qualitative level diagram of the $^7$Li $2^2S_{1/2}$, $2^2P_{1/2}$, and $2^2P_{3/2}$ states (not to scale).](./lithiumlevels.svg){fig-alt="Qualitative level diagram of the $^7$Li $2^2S_{1/2}$, $2^2P_{1/2}$, and $2^2P_{3/2}$ states (not to scale)." fig-align="center" width=40%}
