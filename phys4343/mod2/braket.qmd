...
where $\ket{a}$ is an eigenstate of $\hat A$ and $a$ is the corresponding eigenvalue.

The wave function is the position-space representation of the abstract state,
...
For an orthonormal basis $\{\ket{n}\}$,
...
$$
\boxed{
\sum_n \ketbra{n}{n}=\mathbb 1.
}
$$

A state can therefore be expanded as
...
The matrix elements of an operator in a chosen basis are
...
Their eigenvalues are possible measurement outcomes. If the system is in the normalized state $\ket{\psi}$, the probability of obtaining the eigenvalue $a_n$ is
...
- the abstract state $\ket{\psi}$ is independent of how it is represented: $\psi(x)=\braket{x}{\psi}$ and $\tilde\psi(p)=\braket{p}{\psi}$ are different representations of the same state;
- a bra--ket expression such as $\braket{\phi}{\psi}$ is an **inner product**, while an outer product such as $\ketbra{\phi}{\psi}$ is an **operator**;
...
- the coefficients $\braket{n}{\psi}$ are the projections of a state onto the basis states and determine its expansion in that basis;
