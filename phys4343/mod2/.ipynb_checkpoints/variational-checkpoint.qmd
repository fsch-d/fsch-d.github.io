...
# 1. The Variational Principle in Quaantum Mechanics
...
We start with the expectation value of the Hamiltonian using the trial expansion:
...
where we define the **Hamiltonian** and **overlap** matrix elements as
...
To find the stationary energy with respect to variations in each $c_i^*$, we set
...
This is the **secular equation**, a set of $n$ homogeneous linear equations in the coefficients $\{c_j\}$.

For nontrivial solutions to exist, the determinant of the coefficient matrix must vanish:

$$
\det(H - E S) = 0.
$$

Solving this equation yields $n$ approximate eigenvalues $E_k$ and corresponding coefficient vectors $\mathbf{c}^{(k)}$.
