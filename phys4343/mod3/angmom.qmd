...
To solve the Schrödinger equation for the hydrogen atom, we must understand the angular part of the wave function. This leads naturally to the quantum-mechanical theory of **orbital angular momentum** and to the spherical harmonics $Y_\ell^m(\theta,\varphi)$.

A more general discussion of angular momentum—including the abstract angular-momentum algebra, ladder operators, spin, and the coupling of different angular momenta—is given in [Angular Momenta and Their Couplings](../mod2/amcoupling.qmd).

Some of those general results are repeated here so that the derivation of the hydrogen wave function can be followed without requiring that chapter as a prerequisite.
...
2. Employ separation of variables in spherical coordinates to transform the angular eigenvalue problem into ordinary differential equations.
3. Solve these equations to determine the allowed eigenvalues of $\hat L^2$ and $\hat L_z$ and the corresponding eigenfunctions.
4. Introduce the spherical harmonics $Y_\ell^m(\theta,\varphi)$ as the angular part of the hydrogen wave function.
...
The following commutation relations are a particular realization of the **general angular-momentum algebra** discussed in [Angular Momenta and Their Couplings](../mod2/amcoupling.qmd). Here we derive them directly from $\hat{\vec L}=\hat{\vec r}\times\hat{\vec p}$ so that the orbital-angular-momentum discussion remains self-contained.
...
- Therefore, they cannot in general have simultaneously sharp values, and there is no complete basis of simultaneous eigenstates of all three components.
:::
...
#| fig-format: svg
#| fig-cap: Angular momentum eigenvector in quantum mechanics.
#| fig-alt: Angular momentum eigenvector in quantum mechanics.
...
Ladder operators are developed in a more general setting in [Angular Momenta and Their Couplings](../mod2/amcoupling.qmd). We repeat the essential construction here for **orbital angular momentum** so that this page remains self-contained and so that we can connect the algebra directly to the spherical harmonics.

To understand the structure of the orbital angular-momentum eigenstates, it is useful to introduce the **ladder operators**:
...
- The ladder operators explain the **spacing and finite range** of the $m$ values for a fixed $\ell$:
  ...
  - Each application changes $m$ by one unit, and the ladder terminates at $m=\pm\ell$.
  - The fact that $\ell$ and $m$ are **integers** for orbital angular momentum was established above from the single-valuedness and regularity of the spatial wave functions. In the general theory of angular momentum, half-integer quantum numbers are also possible.
...
These spherical harmonics provide the **angular parts** of the familiar $s$, $p$, and $d$ orbitals in atomic physics; the complete orbital wave functions also contain a radial factor.
